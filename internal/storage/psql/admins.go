...
func (s *Storage) Admins(
	ctx context.Context,
	limit int,
	offset int,
) ([]models.User, error) {
	const op = "storage.psql.Admins"
	...
	builder := s.builder.
		...
		Where(squirrel.And{
			squirrel.Eq{
				"user_statuses.user_status_name": models.UserStatusAuthorized,
			},
			squirrel.Eq{
				"user_roles.user_role_name": models.UserRoleAdmin,
			},
		})
...
func (s *Storage) AdminsCount(
	ctx context.Context,
) (int, error) {
	const op = "storage.psql.AdminsCount"
