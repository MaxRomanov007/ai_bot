...
func (s *Storage) Unauthorized(
	ctx context.Context,
	limit int,
	offset int,
) ([]models.User, error) {
	const op = "storage.psql.Unauthorized"
	...
	builder := s.builder.
		...
		Where(squirrel.And{
			squirrel.Eq{
				"user_statuses.user_status_name": models.UserStatusUnauthorized,
			},
			squirrel.Eq{
...
func (s *Storage) UnauthorizedCount(
	ctx context.Context,
) (int, error) {
	const op = "storage.psql.UnauthorizedCount"
